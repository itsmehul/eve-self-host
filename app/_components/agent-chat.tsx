...
export function AgentChat({
  ...
  const basicPassword = process.env.NEXT_PUBLIC_ROUTE_AUTH_BASIC_PASSWORD;
  const agent = useEveAgent({
    auth:
      basicPassword === undefined || basicPassword.length === 0
        ? undefined
        : {
            basic: {
              username: process.env.NEXT_PUBLIC_ROUTE_AUTH_BASIC_USERNAME ?? "eve",
